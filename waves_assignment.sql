...
-- Creating table for the volume of agent transaction within the week
...
AS
	...
	/*left join to ensure i don't create a record for an agent
	that may not have done any transactions within the week.*/
...
CREATE TABLE send_volume_by_country_kind_1
AS
	SELECT SUM(send_amount_scalar) AS volume, 
		tr.kind,
...
CREATE TABLE send_volume_by_country_kind_2
AS
	SELECT SUM(send_amount_scalar) AS volume,
		tr.kind,
		wallets.ledger_location AS country, --Aliased to suit question deliverables
		COUNT(tr.transfer_id) AS transaction_count,
		COUNT(tr.source_wallet_id) AS unique_senders
	FROM transfers AS tr
		INNER JOIN wallets
		ON tr.source_wallet_id = wallets.wallet_id
	WHERE tr.when_created >= NOW() - interval '1 week'
